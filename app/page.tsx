import Navbar from "@/components/Navbar";

export default function Home() {
  return (
    <div className="min-h-screen flex flex-col bg-black  font-sans dark:bg-black">
      <Navbar />
      <main className="flex-grow container mx-auto px-4 py-8 text-white">
        <h1 className="text-4xl font-bold mb-4">Welcome to Lions</h1>
        <p className="text-lg">
          Your trusted partner in buying and selling cars. Explore our extensive
          inventory or get in touch with us today!
        </p>
        <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Rerum ullam
          consequuntur non fugit eligendi nulla odio, necessitatibus deleniti,
          dignissimos magnam nihil ex dolor vel cum quis minima! Odio,
          architecto a.
        </p>
        <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Rerum ullam
          consequuntur non fugit eligendi nulla odio, necessitatibus deleniti,
          dignissimos magnam nihil ex dolor vel cum quis minima! Odio,
          architecto a.
        </p>
        <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Rerum ullam
          consequuntur non fugit eligendi nulla odio, necessitatibus deleniti,
          dignissimos magnam nihil ex dolor vel cum quis minima! Odio,
          architecto a.
        </p>
        <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Rerum ullam
          consequuntur non fugit eligendi nulla odio, necessitatibus deleniti,
          dignissimos magnam nihil ex dolor vel cum quis minima! Odio,
          architecto a.
        </p>
        <p className="text-lg">
          Your trusted partner in buying and selling cars. Explore our extensive
          inventory or get in touch with us today!
        </p>
        <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Rerum ullam
          consequuntur non fugit eligendi nulla odio, necessitatibus deleniti,
          dignissimos magnam nihil ex dolor vel cum quis minima! Odio,
          architecto a.
        </p>
        <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Rerum ullam
          consequuntur non fugit eligendi nulla odio, necessitatibus deleniti,
          dignissimos magnam nihil ex dolor vel cum quis minima! Odio,
          architecto a.
        </p>
        <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Rerum ullam
          consequuntur non fugit eligendi nulla odio, necessitatibus deleniti,
          dignissimos magnam nihil ex dolor vel cum quis minima! Odio,
          architecto a.
        </p>
        <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Rerum ullam
          consequuntur non fugit eligendi nulla odio, necessitatibus deleniti,
          dignissimos magnam nihil ex dolor vel cum quis minima! Odio,
          architecto a.
        </p>
        <p className="text-lg">
          Your trusted partner in buying and selling cars. Explore our extensive
          inventory or get in touch with us today!
        </p>
        <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Rerum ullam
          consequuntur non fugit eligendi nulla odio, necessitatibus deleniti,
          dignissimos magnam nihil ex dolor vel cum quis minima! Odio,
          architecto a.
        </p>
        <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Rerum ullam
          consequuntur non fugit eligendi nulla odio, necessitatibus deleniti,
          dignissimos magnam nihil ex dolor vel cum quis minima! Odio,
          architecto a.
        </p>
        <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Rerum ullam
          consequuntur non fugit eligendi nulla odio, necessitatibus deleniti,
          dignissimos magnam nihil ex dolor vel cum quis minima! Odio,
          architecto a.
        </p>
        <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Rerum ullam
          consequuntur non fugit eligendi nulla odio, necessitatibus deleniti,
          dignissimos magnam nihil ex dolor vel cum quis minima! Odio,
          architecto a.
        </p>
        <p className="text-lg">
          Your trusted partner in buying and selling cars. Explore our extensive
          inventory or get in touch with us today!
        </p>
        <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Rerum ullam
          consequuntur non fugit eligendi nulla odio, necessitatibus deleniti,
          dignissimos magnam nihil ex dolor vel cum quis minima! Odio,
          architecto a.
        </p>
        <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Rerum ullam
          consequuntur non fugit eligendi nulla odio, necessitatibus deleniti,
          dignissimos magnam nihil ex dolor vel cum quis minima! Odio,
          architecto a.
        </p>
        <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Rerum ullam
          consequuntur non fugit eligendi nulla odio, necessitatibus deleniti,
          dignissimos magnam nihil ex dolor vel cum quis minima! Odio,
          architecto a.
        </p>
        <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Rerum ullam
          consequuntur non fugit eligendi nulla odio, necessitatibus deleniti,
          dignissimos magnam nihil ex dolor vel cum quis minima! Odio,
          architecto a.
        </p>
        <p className="text-lg">
          Your trusted partner in buying and selling cars. Explore our extensive
          inventory or get in touch with us today!
        </p>
        <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Rerum ullam
          consequuntur non fugit eligendi nulla odio, necessitatibus deleniti,
          dignissimos magnam nihil ex dolor vel cum quis minima! Odio,
          architecto a.
        </p>
        <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Rerum ullam
          consequuntur non fugit eligendi nulla odio, necessitatibus deleniti,
          dignissimos magnam nihil ex dolor vel cum quis minima! Odio,
          architecto a.
        </p>
        <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Rerum ullam
          consequuntur non fugit eligendi nulla odio, necessitatibus deleniti,
          dignissimos magnam nihil ex dolor vel cum quis minima! Odio,
          architecto a.
        </p>
        <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Rerum ullam
          consequuntur non fugit eligendi nulla odio, necessitatibus deleniti,
          dignissimos magnam nihil ex dolor vel cum quis minima! Odio,
          architecto a.
        </p>
      </main>
    </div>
  );
}
